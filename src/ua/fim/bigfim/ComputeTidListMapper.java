...
public class ComputeTidListMapper extends
	...
	public void setup(Context context) throws IOException {
		...
		if (localCacheFiles != null) {
			...
			List<Set<Integer>> tmpWords = AprioriPhaseMapper
					.readWordsFromFile(filename);
			singletons = new HashSet<Integer>();
			Set<SortedSet<Integer>> words = createLengthPlusOneWords(tmpWords,
					singletons);
	...
	public void map(LongWritable key, Text value, Context context)
		...
		List<Integer> items = AprioriPhaseMapper.convertLineToSet(line,
				phase == 1, singletons);
	...
	private void reportItemTids(Context context, List<Integer> items)
			throws IOException, InterruptedException {
		if (items.size() < phase) {
			return;
		}
		...
		if (tidCounter >= TIDS_BUFFER_SIZE) {
			System.out.println("Tids buffer reached, reporting " + tidCounter
					+ " partial tids");
			doRecursiveReport(context, new StringBuilder(), countTrie);
			tidCounter = 0;
		}
	...
	private void doRecursiveTidAdd(Context context, List<Integer> items,
			int ix, Trie trie) throws IOException, InterruptedException {
	...
	private void doRecursiveReport(Context context, StringBuilder builder,
			Trie trie) throws IOException, InterruptedException {
		...
		for (Trie recTrie : trie.children.values()) {
			if (recTrie != null) {
				if (recTrie.children.isEmpty()) {
					...
					Text key = new Text(builder.substring(0,
							builder.length() - 1));
					IntWritable[] iw = createIntWritableWithIdSet(recTrie.tids
							.size());
					int i1 = 1;
