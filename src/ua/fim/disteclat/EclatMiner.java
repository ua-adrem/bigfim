...
public class EclatMiner {
	...
	public int mine(java.util.Map<String, Integer> indices, List<Item> bitSets,
		...
		LinkedList<Item> curr = new LinkedList<Item>(
		...
		if (curr.size() < maxSize) {
			...
			ArrayList<Item> diffSets = new ArrayList<Item>(bitSets.size()
					- indexLastItem);
			...
			while (it.hasNext()) {
				...
				if (sup - diffSet.length >= minSup) {
					diffSets.add(new Item(nextItem.name, sup - diffSet.length,
							diffSet));
				}
			}
	...
	public int mine(List<Extension> extensions, List<Item> singletons,
			Map<String, Integer> singletonsIndexMap, String string, int minSup) {
		long beg = System.currentTimeMillis();
		...
		if (curr.size() < maxSize) {
			...
			ArrayList<Item> diffSets = new ArrayList<Item>(singletons.size()
					- index);
			...
			while (it.hasNext()) {
				...
				Item nextItem = singletons.get(singletonsIndexMap
						.get(nextItemName));
			...
			if (pgReporter != null && curr.size() == maxSize - 1
					&& !diffSets.isEmpty()) {
				// create prefixes list
				StringBuilder builder = new StringBuilder();
				for (Item anItem : curr) {
	...
	public int mineNonRec(List<Extension> extensions, List<Item> singletons,
			Map<String, Integer> singletonsIndexMap, String string, int minSup) {
		long beg = System.currentTimeMillis();
	...
	private int declat(int currSup, LinkedList<Item> curr, int index,
			List<Item> bitSets, int minSup) {
		...
		if (curr.size() < maxSize) {
			...
			while (it.hasNext()) {
				...
				if (sup - supOf >= minSup) {
					newBitSets.add(new Item(nextItem.name, sup - supOf,
							condTids));
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (prefix.size() < maxSize) {
					...
					report &= (sup >= minSup && prefix.size() >= minSize && prefix
							.size() < maxSize);
					if (report) {
						found++;
	...
	private static List<Item> readSingletons(Configuration conf, Path path)
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			miner.setSetReporter(new SetReporter.TreeStringReporter(
					new PrintStream("outputFile.txt")));
			int i = 0;
