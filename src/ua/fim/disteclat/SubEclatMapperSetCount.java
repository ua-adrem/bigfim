...
/**
 * This class implements the Mapper for the third MapReduce cycle for
 * Dist-Eclat. It receives a list of prefixes that have to be extended and mined
 * to obtain frequent itemsets.This class only reports the number of itemsets
 * that have been found on each level for each prefix group.
 * 
 * @author Sandy Moens & Emin Aksehirli
 */
public class SubEclatMapperSetCount extends
		SubEclatMapperBase<LongWritable, Text, Text, LongWritable> {
	...
	public void map(LongWritable key, Text value, Context context)
		...
		HadoopPerLevelCountReporter reporter = new HadoopPerLevelCountReporter(
				context);
		...
		while (st.hasMoreElements()) {
			...
			this.time += time;
			i++;
			System.out.println("Prefix: " + i + "/" + prefixes + ", time: "
					+ time + ", avg time: " + 1.0 * this.time / i);
	...
	private void printLevelInfo(HadoopPerLevelCountReporter reporter) {
		...
		for (int level = 0; level < maxLevel; level++) {
			builder.append("Sets for level " + level + ": "
					+ reporter.getCountForLevel(level) + "\n");
		}
		builder.append("=================================================");
