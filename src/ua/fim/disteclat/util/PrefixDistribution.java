 ...
 * should divide the received prefixes in a number of independent groups, while
 * trying to optimize the load balancing.
 * 
...
public interface PrefixDistribution {
	...
	/**
	 * This class implements the Lowest Frequency prefix distribution. It sorts
	 * prefixes in descending order of frequency. Then, when distributing
	 * prefixes, it continuously gives a new prefix to the group with the
	 * overall lowest support.
	 * 
	 * @author Sandy Moens & Emin Aksehirli
	 */
	public class LowestFrequencyPDistribution implements PrefixDistribution {
		...
		@Override
		public void sortPrefixes(List<ItemPair> prefixes,
				final Map<String, Integer> supMap) {
		...
		@Override
		public void writeDistributionToOutput(int numberOfMappers,
				List<ItemPair> prefixes, MultipleOutputs<Text, Text> mos,
			...
			for (int ix = 0; ix < end; ix++) {
				mos.write(
						outputName,
						new Text("" + ix),
						new Text(builders[ix].substring(0,
								builders[ix].length() - 1)));
			}
		}
	...
	/**
	 * This class implements the Round Robing prefix distribution. It sorts
	 * prefixes in ascending order of constituting singletons. Then, it divides
	 * the prefixes among k mappers using the formula i%k=mId, where i is the
	 * index of the prefix, and mId is the id of the mapper
	 * 
	 * @author Sandy Moens & Emin Aksehirli
	 */
	public class RoundRobinPDistribution implements PrefixDistribution {
		...
		public void writeDistributionToOutput(int numberOfMappers,
			...
			for (int ix = 0; ix < end; ix++) {
				...
				mos.write(outputName, new Text("" + ix),
