...
public interface SetReporter {
			...
			try {
				for (Entry<Integer, AtomicLong> entry : counts.entrySet()) {
					context.write(new Text("" + entry.getKey()),
							new LongWritable(entry.getValue().get()));
				}
			} catch (IOException e) {
				e.printStackTrace();
	...
	public static class HadoopPerLevelReporter implements SetReporter {
		...
		public HadoopPerLevelReporter(Context context, String fisKey,
				String prefixKey, int prefixLength) {
			this.context = context;
			this.fisKey = fisKey;
		...
		public HadoopPerLevelReporter(Context context, String fis,
				String prefix, int prefixLength, String delimiter) {
		...
		public void report(List<Item> itemset, int support, int[] ints) {
			...
			try {
				...
				context.write(new Text(prefixKey + delimiter + itemString
						+ delimiter + support), new ObjectWritable(ints));
			} catch (IOException e) {
				e.printStackTrace();
			} catch (InterruptedException e) {
				e.printStackTrace();
		...
		@Override
		public void close() {
			for (Entry<Integer, List<String>> entry : freqSetsPerLevel
					.entrySet()) {
				StringBuilder builder = new StringBuilder();
				...
				try {
					context.write(new Text(fisKey),
							new ObjectWritable(builder.toString()));
				} catch (IOException e) {
					e.printStackTrace();
				} catch (InterruptedException e) {
					e.printStackTrace();
	...
	public class HadoopTreeStringReporter implements SetReporter {
		...
		public void report(List<Item> itemset, int support) {
			if (prevSet == null) {
				...
				int depth = 0;
				while (depth < itemset.size()
						&& depth < prevSet.size()
						&& itemset.get(depth).name
								.equals(prevSet.get(depth).name)) {
					depth++;
				}
			...
			builder.append(itemset.get(itemset.size() - 1).name + OPENSUP
					+ support + CLOSESUP);
