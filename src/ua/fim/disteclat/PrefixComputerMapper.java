...
/**
 * This class implements the Mapper for the second MapReduce cycle for
 * Dist-Eclat. It receives a list of singletons for which it has to create X-FIs
 * seeds by growing the lattice tree downwards. The ordering is retrieved
 * through the distributed cache.
 * 
 * @author Sandy Moens & Emin Aksehirli
 */
public class PrefixComputerMapper extends
		...
		try {
			...
			for (Path path : localCacheFiles) {
				...
				if (pathString.contains(OSingletonsTids)) {
					System.out
							.println("[PrefixComputerMapper]: Reading singletons");
					singletons = readTidLists(conf, path);
				} else if (pathString.contains(OSingletonsOrder)) {
					System.out
							.println("[PrefixComputerMapper]: Reading singleton orders");
	...
	public void map(LongWritable key, Text value, Context context)
		...
		if (prefixLength == 1) {
			...
			HadoopPerLevelReporter reporter = new HadoopPerLevelReporter(
					context, Fis, Prefix, prefixLength, Delimiter);
