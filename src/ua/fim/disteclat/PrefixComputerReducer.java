 ...
 * Dist-Eclat. It gets all singletons, frequent itemsets to level X and all
 * seeds. Singletons are used to sort the seeds based on ascending frequency of
 * the singletons they contain. The seeds are then distributed among the
 * available number of mappers and written to disk.
 * 
 * @author Sandy Moens & Emin Aksehirli
 */
public class PrefixComputerReducer extends
	...
	public void cleanup(Context context) throws IOException,
		...
		pDistribution.writeDistributionToOutput(numberOfMappers, prefixes, mos,
				OPrefixesDistribution);

		mos.close();
	}

	/**
	 * Parses a list of singletons with their support and puts them in the
	 * singletons support map.
	 * 
	 * @param values
	 ...
	 */
	private void handleFis(Iterable<ObjectWritable> values) throws IOException,
			InterruptedException {
		StringBuilder builder = new StringBuilder();
		for (Iterator<ObjectWritable> it = values.iterator(); it.hasNext();) {
			builder.append((String) it.next().get());
		}
		mos.write(DistEclatDriver.OFises, new Text(),
				new Text(builder.substring(0, builder.length() - 1)));
	}

	/**
	 * Parses a prefix group together with its extensions. The last extension
	 * can also contain a list of subextensions, which means the former is
	 * actually a closed extension.
	 * 
	 * @param values
	 ...
	 * @throws InterruptedException
	 */
	private void handlePrefixGroups(Iterable<ObjectWritable> values,
			String prefixString) throws IOException, InterruptedException {
		...
		for (ObjectWritable value : values) {
			...
			// write group to file
			mos.write(DistEclatDriver.OPrefixesGroups, new Text(prefixString),
					new Text((String) value.get()));
		}
	}

	/**
	 * Creates a list of prefixes based on the frequent singletons and puts them
	 * into prefixes.
	 */
	private void createSingletonPrefixes() {
		for (Entry<String, Integer> entry : supMap.entrySet()) {
			prefixes.add(new ItemPair(new String[] { entry.getKey() }, entry
					.getValue()));
		}
	}

	/**
	 * Writes the empty prefix group. This is only used when prefixes of length
	 * 1 have been specified. The empty prefix group consists of an empty prefix
	 * and all frequent singletons.
	 * 
	 * @throws IOException
	 * @throws InterruptedException
	 */
	private void writeEmptyPrefixGroup() throws IOException,
			InterruptedException {
		StringBuilder builder = new StringBuilder();
		for (ItemPair singleton : getSortedSingletonPrefixes()) {
			...
			builder.append(name.substring(0, name.length() - 1)
					+ DELIMITER_SUPPORT + support + " ");
		}
		// write group to file
		mos.write(DistEclatDriver.OPrefixesGroups, new Text(""), new Text(
				builder.substring(0, builder.length() - 1)));
	}

	/**
	 * Returns the sorted list of singleton prefixes in ascending order of
	 * frequency.
	 * 
