...
/**
 * Input format that splits a file in a number of chunks given by
 * Config.NUMBER_OF_MAPPERS_KEY.
 * 
 * @author Sandy Moens & Emin Aksehirli
 */
public class SplitByNumberOfMappersTextInputFormat extends
	...
	/**
	 * Gets the different file splits for the data based on a given number of
	 * splits
	 * 
	 * @param status
	 *            file status
	 ...
	 */
	private static List<FileSplit> getSplitsForFile(FileStatus status,
			Configuration conf, int numberOfSplits) throws IOException {
		...
		int numLinesPerSplit = (int) Math.ceil(1.0 * totalNumberOfLines
				/ numberOfSplits);
	...
	/**
	 * Gets the total number of lines from the file. If
	 * Config.NUMBER_OF_LINES_KEY is set, this value is returned.
	 * 
	 * @param conf
	 *            hadoop configuration object
	 * @param fileName
	 ...
	 * @throws IOException
	 */
	private static long getTotalNumberOfLines(Configuration conf, Path fileName)
			throws IOException {
